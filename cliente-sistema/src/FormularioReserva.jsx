...
export default function FormularioReserva({ usuarioLogueado }) {
  ...
  const [datosExito, setDatosExito] = useState(null); // ✅ PASO 2: estado nuevo
  ...
  const ejecutarPeticionBackend = (payload) => {
    ...
    .then(data => {
      // ✅ PASO 3: reemplaza el alert por la pantalla de éxito
      const resumenReserva = {
  ...payload,
  codigoReserva: data.codigoReserva || data.codigo_reserva || payload.codigoReserva,
  mesaNumero: mesaSeleccionada?.numero,
  usuarioNombre: (usuarioLogueado || JSON.parse(localStorage.getItem('usuarioLogueado') || '{}')).nombre,
  // ✅ Genera código de estacionamiento solo si lo pidió
  codigoEstacionamiento: payload.estacionamiento === 1 
    ? 'EST-' + Math.random().toString(36).substring(2, 5).toUpperCase() 
    : null,
};
      setDatosExito(resumenReserva);
      setMesasBackend(prev => prev.map(m => m.id === payload.mesa.id ? { ...m, estado: 'reservada' } : m));
      setMesaSeleccionada(null);
      setMostrarModalPago(false);
      setPayloadTemporal(null);
      setDatosForm({ fecha: '', hora: '', capacidad: 2, metodoPago: 'Efectivo', estacionamiento: 0 });
      setEnviando(false);
    })
    .catch(err => {
  ...
  metodoPago: datosForm.metodoPago.toLowerCase().includes('efectivo') ? 'efectivo'
            : datosForm.metodoPago.toLowerCase().includes('tarjeta')  ? 'tarjeta'
            : 'yape_plin',
  estadoPago: datosForm.metodoPago.toLowerCase().includes('efectivo') ? 'pendiente' : 'pendiente',
      ...
      {mostrarModalPago && (
        <ModalPago 
          ...
          onConfirmar={(respuestaPasarela) => {
            const estadoPagoFinal = respuestaPasarela.estadoPago === 'APROBADO' ? 'pagado' : 'pendiente';
            if (!payloadTemporal) {
            ...
            const payloadFinal = {
  ...payloadTemporal,
  codigoReserva: payloadTemporal.codigoReserva, // ✅ Mantiene el PAN-XXXX original
  codigoOperacionPago: respuestaPasarela.codigoOperacion, // ✅ Guarda el número Yape aparte
  estadoPago: respuestaPasarela.estadoPago === 'APROBADO' ? 'pagado' : 'pendiente'
};
      ...
      {/* ✅ PASO 4: PANTALLA DE ÉXITO CON VOUCHER PDF */}
      {datosExito && (
        <PantallaExito
          datos={datosExito}
          onCerrar={() => setDatosExito(null)}
        />
      )}
